...
class NotificationManager {
  ...
  Future<void> scheduleTaskStartNotification(Task task, ScheduledTask scheduledTask, DateTime scheduledTime) async {
    ...
    try {
      switch (scheduledTask.notification) {
        ...
        case NotificationType.push:
          ...
          logInfo('Would schedule push notification for task start: ${task.title} at $scheduledTime');
          break;
  ...
  Future<void> scheduleTaskReminderNotification(Task task, ScheduledTask scheduledTask, DateTime scheduledTime, Duration timeBeforeStart) async {
    if (!_isInitialized) await initialize();
    ...
    try {
      switch (scheduledTask.notification) {
        ...
        case NotificationType.push:
          // Push notification
          logInfo('Would schedule push notification for task reminder: ${task.title} at $scheduledTime');
          break;
        case NotificationType.email:
          // Email notification
          _scheduleEmailNotification(
  ...
  Future<void> notifyTaskReminder(Task task, ScheduledTask scheduledTask, Duration timeBeforeStart) async {
    if (!_isInitialized) await initialize();
    ...
    try {
      switch (scheduledTask.notification) {
        ...
        case NotificationType.both:
          // Local notification
          await _localNotificationService.notifyTaskReminder(task, scheduledTask, timeBeforeStart);
          break;
        case NotificationType.push:
          // Push notification
          await _pushNotificationService.notifyTaskReminder(task, scheduledTask, timeBeforeStart);
          break;
        case NotificationType.email:
          // Email notification
          await _emailNotificationService.notifyTaskReminder(task, scheduledTask, timeBeforeStart);
          break;
        case NotificationType.pushAndEmail:
          // Both push and email
          await _pushNotificationService.notifyTaskReminder(task, scheduledTask, timeBeforeStart);
  ...
  void _scheduleEmailNotification(
    ...
    logInfo('Would schedule email notification for task $actionType: ${task.title} at $scheduledTime');
